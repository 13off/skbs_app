...
class DispatcherSummarySettings {
  ...
  factory DispatcherSummarySettings.fromJson(Map<String, dynamic> json) {
    return DispatcherSummarySettings(
      ...
      recipientRoles: _stringSet(json['recipient_roles'], const <String>{'admin'}),
  ...
  static Set<int> _intSet(dynamic value, Set<int> fallback) {
    ...
    final result = value.map((item) => int.tryParse(item.toString())).whereType<int>().toSet();
    return result.isEmpty ? fallback : result;
  }

  static Set<String> _stringSet(dynamic value, Set<String> fallback) {
    if (value is! List) return fallback;
    final result = value.map((item) => item.toString()).where((item) => item.isNotEmpty).toSet();
    return result.isEmpty ? fallback : result;
  }
}
...
class DispatcherSummaryCenter {
  ...
  const DispatcherSummaryCenter({required this.settings, required this.runs});

  factory DispatcherSummaryCenter.fromJson(Map<String, dynamic> json) {
    final settings = _map(json['settings']);
    final runs = json['runs'] is List ? json['runs'] as List : const <dynamic>[];
    return DispatcherSummaryCenter(
      settings: settings.isEmpty
          ? DispatcherSummarySettings.defaults
          : DispatcherSummarySettings.fromJson(settings),
      runs: runs.map((item) => DispatcherSummaryRun.fromJson(_map(item))).toList(),
    );
  }
}
...
class DispatcherSummaryRepository {
  ...
  static Future<DispatcherSummarySettings> save(DispatcherSummarySettings settings) async {
