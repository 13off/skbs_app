...
class _StroyNaVekaPainter extends CustomPainter {
  ...
  void _drawShield(Canvas canvas) {
    ...
    final fill = Paint()
      ..color = _blue.withValues(alpha: 0.12 + (0.88 * p))
  ...
  void _drawCentralWall(Canvas canvas) {
    ...
    for (var row = 0; row < rows; row++) {
      ...
      final rowP = _interval(p, rowStart, math.min(1, rowStart + 0.42));
      if (rowP <= 0) continue;
      final y = bottom - ((row + 1) * brickH);
      final stagger = row.isOdd ? 6.0 : 0.0;
      final widths = row.isOdd ? <double>[18, 24, 24, 18] : <double>[22, 22, 22, 16];
      var x = left - stagger;
      for (var i = 0; i < widths.length; i++) {
        ...
        final rect = Rect.fromLTWH(x, y + (8 * (1 - local)), widths[i] - 1.5, brickH - 1.5);
    ...
    if (battlementP > 0) {
      for (var i = 0; i < 5; i++) {
        ...
        final rect = Rect.fromLTWH(81 + (i * 15), 75 + (7 * (1 - local)), 10, 12);
        _drawBrick(canvas, rect, local);
      }
    }
  }
  ...
  void _drawTower(Canvas canvas, {required bool left}) {
    ...
    for (var row = 0; row < rows; row++) {
      ...
      final rowP = _interval(p, start, math.min(1, start + 0.36));
      if (rowP <= 0) continue;
      final y = bottom - ((row + 1) * brickH);
      ...
      _drawBrick(
        canvas,
        Rect.fromLTWH(cx - (width / 2) - offset, y + (10 * (1 - rowP)), firstW + offset - 1.4, brickH - 1.3),
        rowP,
      );
      _drawBrick(
        canvas,
        Rect.fromLTWH(cx - (width / 2) + firstW, y + (10 * (1 - rowP)), secondW + offset - 1.4, brickH - 1.3),
    ...
    if (crownP > 0) {
      ...
      canvas.drawRRect(
        cap,
        Paint()..color = _white.withValues(alpha: crownP),
      );
      canvas.drawRRect(
        cap,
        Paint()
  ...
  void _drawRoofs(Canvas canvas) {
    ...
    for (final left in <bool>[true, false]) {
      final cx = left ? 59.0 : 177.0;
      final sideDelay = left ? 0.0 : 0.07;
      final local = _interval(p, sideDelay, 1);
      if (local <= 0) continue;
      ...
      canvas.drawPath(
        roof,
        Paint()..color = _white.withValues(alpha: local),
      );
      canvas.drawPath(
        roof,
        Paint()
  ...
  void _drawTitleBlock(Canvas canvas) {
    ...
    canvas.drawPath(
      panel,
      Paint()..color = _blueDark.withValues(alpha: 0.96 * p),
    );
    ...
    canvas.drawLine(Offset(72, 213), Offset(107, 213), linePaint);
    canvas.drawLine(Offset(129, 213), Offset(164, 213), linePaint);
    final diamond = Path()
      ..moveTo(118, 207)
      ..lineTo(124, 213)
      ..lineTo(118, 219)
  ...
  void _drawTowerWindow(Canvas canvas, double cx, double y, double p) {
    ...
    canvas.drawPath(
      window,
      Paint()..color = _blueDark.withValues(alpha: 0.9 * p),
    );
  }
  ...
  void _drawGroundSpark(Canvas canvas) {
    final p = _ease(1 - settleProgress);
    if (p <= 0.02 || towerProgress <= 0.05) return;
    final active = _clamp01(towerProgress * (1 - towerProgress) * 3.4) * p;
    if (active <= 0) return;
    final paint = Paint()
