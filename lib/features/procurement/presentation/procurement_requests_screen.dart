...
class _ProcurementRequestsScreenState extends State<ProcurementRequestsScreen> {
  ...
  Future<List<ProcurementRequest>> load() => ProcurementRepository.fetchRequests(
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Отменить заявку')),
        ],
      ),
    );
  ...
  Color statusColor(ProcurementRequest request) {
    ...
    if (request.isOverdue || request.priority == 'urgent') return AppAdaptivePalette.danger;
  ...
  List<ProcurementRequest> visible(List<ProcurementRequest> source) {
    ...
    return source.where((item) {
      ...
      final haystack = '${item.title} ${item.objectName} ${item.supplierName}'.toLowerCase();
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(request.objectName, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: AppAdaptivePalette.textMuted, fontWeight: FontWeight.w700)),
                    ],
                  ),
                ),
                PopupMenuButton<String>(
                  ...
                  itemBuilder: (_) => <PopupMenuEntry<String>>[
                    if (!request.isClosed) const PopupMenuItem(value: 'edit', child: Text('Редактировать')),
                    if (!request.isClosed) const PopupMenuItem(value: 'cancel', child: Text('Отменить')),
                  ],
                ),
              ],
            ),
            ...
            Wrap(
              ...
              children: [
                ...
                _Chip(label: request.priorityTitle, color: AppAdaptivePalette.textMuted),
                _Chip(label: money(request.totalAmount), color: AppAdaptivePalette.textPrimary),
                _Chip(label: '${request.items.length} поз.', color: AppAdaptivePalette.textMuted),
              ],
            ),
            if (request.supplierName.isNotEmpty) ...[
              const SizedBox(height: 10),
              Text(request.supplierName, style: TextStyle(color: AppAdaptivePalette.textMuted, fontWeight: FontWeight.w700)),
            ],
      ...
      child: Column(
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                ChoiceChip(label: const Text('В работе'), selected: filter == 'open', onSelected: (_) => setState(() => filter = 'open')),
                const SizedBox(width: 8),
                ChoiceChip(label: const Text('Доставка'), selected: filter == 'delivery', onSelected: (_) => setState(() => filter = 'delivery')),
                const SizedBox(width: 8),
                ChoiceChip(label: const Text('Закрытые'), selected: filter == 'closed', onSelected: (_) => setState(() => filter = 'closed')),
                const SizedBox(width: 8),
                ChoiceChip(label: const Text('Все'), selected: filter == 'all', onSelected: (_) => setState(() => filter = 'all')),
              ],
            ),
          ),
          const SizedBox(height: 14),
