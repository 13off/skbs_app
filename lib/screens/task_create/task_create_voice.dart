...
final _voiceAssigneeMarker = RegExp(
...
List<String> _buildTaskVoiceHints(List<Employee> employees) {
  ...
  // Фамилии идут первыми намеренно. Safari ограничивает полезный объём JSGF,
  // поэтому при большом объекте имена сотрудников не должны вытесняться
  // строительным словарём.
  return <String>[
    ...surnames,
    ...fullNames,
...
List<String> _resolveVoiceEmployeeIds({
  ...
  for (var index = 0; index < rawTokens.length; index += 1) {
    ...
    if (current.length >= 3 && !_voiceNameStopWords.contains(current)) {
      tokens.add(current);
    }
    if (index + 1 < rawTokens.length) {
      final next = _normalizeVoiceName(rawTokens[index + 1]);
      final joined = '$current$next';
      if (current.length >= 3 && next.length >= 2 && joined.length <= 18) {
        tokens.add(joined);
      }
    }
  }
  ...
  for (final token in tokens) {
    ...
    for (final candidate in candidates) {
      ...
      if (surnameCounts[candidate.surname] == 1) {
        score = _voiceNameMatchScore(token, candidate.surname);
      }
...
double _voiceNameMatchScore(String heard, String expected) {
  ...
  for (final form in _voiceNameForms(expected)) {
    ...
    if (heardKey.length >= 3 && formKey.length >= 3) {
      final phoneticLongest = heardKey.length > formKey.length
          ? heardKey.length
          : formKey.length;
      final phoneticSimilarity =
          1 - _voiceEditDistance(heardKey, formKey) / phoneticLongest;
...
String _normalizeVoiceName(String value) => value
    .toLowerCase()
    .replaceAll('ё', 'е')
  ...
  Future<void> captureVoiceTask() async {
    ...
    try {
      final transcript = await recognizeTaskVoice(
        hints: _buildTaskVoiceHints(employees),
      ...
      final assigneeIds = _resolveVoiceEmployeeIds(
        transcript: transcript,
        employees: employees,
        fallbackIds: parsed.assigneeIds,
      );
      if (!mounted) return;
  ...
  void applyVoicePartial(String transcript) {
    ...
    setState(() {
      ...
      if (askedForAssignee && assigneeIds.isEmpty) {
        voiceHasWarning = true;
        voiceMessage =
            'Фамилию пока не понял. Повторите: «исполнитель Фамилия». Остальные поля уже сохраняются.';
        ...
        voiceMessage =
            'Готово: ${found.join(' • ')}. Ещё нужно: ${missing.join(' • ')}.';
      }
    });
  }
