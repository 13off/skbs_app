...
if marker not in text:
    raise RuntimeError('Не найден updateMemberAccess')
...
if save_marker not in text:
    raise RuntimeError('Не найден save')
...
button_addition = """          PremiumActionButton(
            onPressed: isSaving ? null : save,
            icon: isEditing ? Icons.save_outlined : Icons.link_rounded,
            label: isEditing ? 'Сохранить права' : 'Создать ссылку',
            isLoading: isSaving,
...
text = text.replace(button_marker, button_addition, 1)
