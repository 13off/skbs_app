...
void main() {
  ...
  test('редактор задачи разделён по ответственности', () {
    ...
    expect(shell, contains("part 'task_details_photo_actions.dart';"));
    ...
    expect(photoActions, contains('Future<void> addPhotosFast(String photoStage)'));
    expect(photoActions, contains('Widget buildFastPhotosBlock('));
    expect(actions, contains('Future<void> saveChanges()'));
    expect(actions, contains('void changeMilestone('));
    expect(sections, contains('Widget buildPhotoTile('));
    expect(sections, contains('Widget buildMilestoneSection()'));
    expect(view, contains('Widget buildTaskDetailsView()'));
    expect(view, contains('buildFastPhotosBlock('));
  });
