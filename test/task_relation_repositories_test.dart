...
void main() {
  test(
    'идентификаторы исполнителей очищаются и сравниваются как множества',
    () {
      expect(
        TaskAssigneeRepository.cleanIdSet(const <String>[
          ' employee-1 ',
          '',
          'employee-1',
          'employee-2',
  ...
  test('данные исполнителя сохраняют прежнее преобразование Supabase', () {
    final assignee = TaskAssigneeData.fromSupabase(<String, dynamic>{
  ...
  test(
    'TaskRepository сохраняет совместимые прокси без прямых relation-запросов',
    () {
      ...
      expect(milestoneRepository, contains(".delete().eq('task_id', taskId)"));
    },
  );
}
