...
void main() {
  ...
  test('v15 выполняется после action trace и до ссылочных и одиночных роутеров', () {
  ...
  test('табель строится по employees плюс attendance и сохраняет объект прораба', () {
  ...
  test('табель понимает отсутствие присутствие и должность внутри найденной группы', () {
  ...
  test('HR умеет фильтровать отсутствие ответственного и неполный комплект', () {
  ...
  test('HR действия переиспользуют проверенные builders вместо прямых записей', () {
  ...
  test('параметры плана не содержат entity ids и остаются диагностическими', () {
  ...
  test('planner сначала отделяет selector от action чтобы не путать целевой статус с фильтром', () {
    expect(planner, contains('selectorClause'));
    expect(planner, contains('actionMarkerIndex'));
    expect(planner, contains('const selector = selectorClause(prompt, "candidates")'));
    expect(planner, contains('const selector = selectorClause(prompt, "procurement")'));
  });
}
