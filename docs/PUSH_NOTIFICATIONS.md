...
- JWT-защищённая Edge Function `manage-push-device` регистрирует, обновляет, отключает и удаляет только устройство текущего пользователя.
- `user_id` берётся из проверенного JWT, а `company_id` — из активного профиля и действующего членства; клиент не может передать эти поля.
- При смене пользователя или компании один FCM-токен безопасно перепривязывается сервером.
- Недействительный FCM-токен с кодом `UNREGISTERED` автоматически отключается.
- В публичной схеме нет клиентских `SECURITY DEFINER`-RPC для управления push-токенами.
