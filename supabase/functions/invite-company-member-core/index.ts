...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    const mustSetPasswordValue = invitedUser?.user_metadata?.must_set_password;
    ...
    if (!existingMembership) {
      ...
      if ((countResult.count ?? 0) >= Number(company.seat_limit)) {
    ...
    if (!invitedUser) {
      ...
      if (!tokenHash) throw new Error("Supabase не вернул токен приглашения");
      actionLink = invitationActionUrl(companyId, tokenHash, verificationType);
    } else {
      const linkType = requiresPasswordSetup ? "recovery" : "magiclink";
      const { data: linkData, error: linkError } =
        await adminClient.auth.admin.generateLink({ type: linkType, email });
      if (linkError) throw linkError;
      const tokenHash = linkData.properties?.hashed_token ?? "";
      const verificationType =
        linkData.properties?.verification_type ?? linkType;
      if (!tokenHash) throw new Error("Supabase не вернул токен входа");
      actionLink = invitationActionUrl(companyId, tokenHash, verificationType);
    ...
    if (role === "foreman") {
      const { error } = await adminClient.from("object_memberships").insert({
        company_id: companyId,
        object_id: objectId,
        user_id: invitedUser.id,
        created_by: actor.id,
    ...
    return json({
      ...
      redirect_to: invitationRedirectUrl(companyId),
    });
  } catch (error) {
    console.error(error);
