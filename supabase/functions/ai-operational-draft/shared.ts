...
export function kind(prompt: string): string {
  ...
  if (/(?:найд|покаж|проверь|есть ли).*(?:расхожд|ошиб|проблем|пропуск|пуст).*(?:табел|смен)/.test(value) ||
      /(?:расхожд|ошиб|проблем|пропуск|пуст).*(?:табел|смен)/.test(value) ||
      /(?:кому|у кого).*(?:нет|не хватает|нул).*(?:смен|табел)/.test(value)) return "find_timesheet_gaps";
