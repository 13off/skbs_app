...
async function uploadPdf(admin: any, app: any, fileNameRaw: string, raw: string) {
  const fileName = safeName(fileNameRaw);
  const bytes = decodePdf(raw);
...
async function cleanup(admin: any, path?: string) {
  if (!path) return;
  try { await admin.storage.from(BUCKET).remove([path]); } catch (_) {}
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "create_flight") {
      for (const field of ["departure_at", "origin", "destination", "file_base64"]) {
        if (!String(body[field] ?? "").trim()) return json({ error: `${field} required` }, 400);
      }
      ...
      if (existing) return json({ ok: true, reused: true, flight_id: existing.id, ...existing });

      const uploaded = await uploadPdf(admin, app, String(body.file_name ?? "ticket.pdf"), String(body.file_base64));
      try {
    ...
    if (action === "attach_ticket") {
      ...
      const { data: existing, error: existingError } = await admin
        ...
        .maybeSingle();
      if (existingError) throw existingError;
      if (existing) return json({ ok: true, reused: true, ticket_id: existing.id, ...existing });

      const uploaded = await uploadPdf(admin, app, originalName, raw);
      try {
        ...
        return json({ ok: true, created: true, ticket_id: ticket.id, flight_id: flightId, ...uploaded }, 201);
      } catch (error) {
        await cleanup(admin, uploaded.path);
        throw error;
      }
