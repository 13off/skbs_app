...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (!token) return json({ error: "FCM token обязателен" }, 400);
    if (!["android", "ios", "web"].includes(platform)) {
      return json({ error: "Недопустимая платформа" }, 400);
    }
    ...
    return json({ error: message || "Не удалось обновить устройство" }, 500);
  }
});
