...
export async function searchCore({
  ...
  if (flags.employees || flags.broad) {
    const found = ranked(
      ...
      if (/архивн|уволен/.test(normalized)) return Boolean(employee.archived_at) || employee.is_active !== true;
      if (/активн/.test(normalized)) return employee.is_active === true && !employee.archived_at;
      return true;
    });
  ...
  if (flags.objects || flags.broad) {
    const found = ranked(
      objects,
      tokens,
      (object: any) => [object.name, object.address, object.comment, object.is_active ? "активный" : "архивный"],
      20,
    );
  ...
  const [tasks, attendance] = await Promise.all([
    flags.tasks || flags.broad ? dataOrEmpty(tasksQuery, "search tasks") : Promise.resolve([]),
    flags.attendance || flags.broad ? dataOrEmpty(attendanceQuery, "search attendance") : Promise.resolve([]),
  ]);
  ...
  if (flags.tasks || flags.broad) {
    const found = ranked(
      tasks,
      tokens,
      (task: any) => [task.task_date, task.object_name, task.axes, task.work, task.status, task.created_by, task.not_done_comment],
      20,
    ).filter((task: any) => {
  ...
  if (flags.attendance || flags.broad) {
    ...
    const found = ranked(
      ...
      (row: any) => {
        ...
        return [employee?.fio, employee?.position, row.work_date, row.object_name, row.status, row.comment, row.marked_by, row.shifts, row.hours];
      },
      25,
    );
  ...
  if (/телефон|номер телефона|ставк|паспорт/.test(normalized)) {
    warnings.push("Чувствительные данные не выводятся в общем чате. Открой карточку сотрудника с соответствующими правами.");
  }
