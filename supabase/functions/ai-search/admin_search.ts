...
export async function searchAdmin({
  ...
  if (!isAdmin) {
    if (flags.payments || flags.receipts || flags.users || flags.company || flags.invitations) {
      warnings.push("Финансы, управление компанией и приглашения доступны в поиске только администратору.");
    }
    return { sections, highlights, warnings };
  }

  const employeeById = new Map<string, any>(employees.map((employee: any) => [String(employee.id), employee]));
  const scopedIds = employees.map((employee: any) => employee.id).filter(Boolean);
  ...
  if (flags.payments || flags.receipts || flags.broad) {
    ...
    if (objectName && scopedIds.length > 0) query = query.in("employee_id", scopedIds);
  ...
  if (flags.payments || flags.broad) {
    const found = ranked(
      ...
      (payment: any) => {
        ...
        return [employee?.fio, employee?.position, payment.payment_date, payment.amount, payment.payment_type, payment.comment, payment.period_year, payment.period_month];
      },
      25,
    ).filter((payment: any) => !matchedEmployee || String(payment.employee_id) === String(matchedEmployee.id));
    ...
    if (lines.length > 0) {
      const total = found.reduce((sum: number, payment: any) => sum + num(payment.amount), 0);
  ...
  if (flags.receipts || flags.broad) {
    ...
    const found = ranked(
      ...
      (receipt: any) => {
        ...
        return [employee?.fio, receipt.file_name, receipt.content_type, receipt.created_at];
      },
      15,
    ).filter((receipt: any) => !matchedEmployee || String(receipt.employee_id) === String(matchedEmployee.id));
  ...
  if (flags.users || flags.broad) {
    const memberships = await dataOrEmpty(
      client.from("company_memberships").select("user_id, role, is_active, created_at").eq("company_id", companyId).order("created_at"),
      "search memberships",
    );
    const ids = memberships.map((item: any) => item.user_id).filter(Boolean);
    const profiles = ids.length > 0
      ? await dataOrEmpty(client.from("user_profiles").select("id, email, full_name, object_name, is_active").in("id", ids), "search profiles")
      : [];
    const profileById = new Map<string, any>(profiles.map((item: any) => [String(item.id), item]));
    const joined = memberships.map((item: any) => ({ ...item, profile: profileById.get(String(item.user_id)) }));
    const found = ranked(
      joined,
      tokens,
      (item: any) => [item.profile?.full_name, item.profile?.email, item.role, item.role === "admin" || item.role === "owner" ? "админ" : "прораб", item.profile?.object_name],
      20,
    ).filter((item: any) => {
      if (/админ/.test(normalized)) return item.role === "admin" || item.role === "owner";
  ...
  if (flags.company || flags.broad) {
    const companies = await dataOrEmpty(
      client.from("companies").select("name, status, plan_code, billing_status, seat_limit, object_limit").eq("id", companyId).limit(1),
      "search company",
    );
  ...
  if (flags.invitations || flags.broad) {
    const invitations = await dataOrEmpty(
      client.from("company_invitations").select("email, role, object_id, status, expires_at").eq("company_id", companyId).order("created_at", { ascending: false }).limit(200),
      "search invitations",
    );
    const objectById = new Map<string, any>(objects.map((object: any) => [String(object.id), object]));
    const found = ranked(
      invitations,
      tokens,
      (invite: any) => [invite.email, invite.role, invite.status, invite.status === "pending" ? "ожидает ожидание" : "", objectById.get(String(invite.object_id))?.name],
      20,
    );
