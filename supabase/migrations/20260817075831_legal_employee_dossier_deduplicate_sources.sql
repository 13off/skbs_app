-- Один физический/сгенерированный кадровый документ должен отображаться один раз.
-- Если recruitment document/form уже перенесён в employee_document_files, исходная
-- запись не добавляется второй раз в досье и счётчики.
...
begin
  ...
  with docs as (
    ...
    where rd.company_id = v_company_id
      ...
      and not exists (
        select 1 from public.employee_document_files edf
        where edf.company_id = rd.company_id
          and edf.employee_id = p_employee_id
          and edf.recruitment_document_id = rd.id
      )
    ...
    where f.company_id = v_company_id
      ...
      and not exists (
        select 1 from public.employee_document_files edf
        where edf.company_id = f.company_id
          and edf.employee_id = p_employee_id
          and edf.recruitment_onboarding_form_id = f.id
      )
...
begin
  ...
  select
    ...
    (
      ...
      + (select count(*) from public.recruitment_documents rd join public.recruitment_applications ra on ra.id=rd.application_id and ra.company_id=rd.company_id where rd.company_id=e.company_id and ra.employee_id=e.id and not exists (select 1 from public.employee_document_files edf where edf.company_id=rd.company_id and edf.employee_id=e.id and edf.recruitment_document_id=rd.id))
      + (select count(*) from public.recruitment_onboarding_forms f where f.company_id=e.company_id and f.employee_id=e.id and not exists (select 1 from public.employee_document_files edf where edf.company_id=f.company_id and edf.employee_id=e.id and edf.recruitment_onboarding_form_id=f.id))
    ...
    (
      ...
      + (select count(*) from public.recruitment_onboarding_forms f left join lateral (select t.code,t.title from public.document_templates t where t.code=f.form_code and (t.company_id is null or t.company_id=f.company_id) order by (t.company_id=f.company_id) desc,t.updated_at desc limit 1) dt on true where f.company_id=e.company_id and f.employee_id=e.id and not exists (select 1 from public.employee_document_files edf where edf.company_id=f.company_id and edf.employee_id=e.id and edf.recruitment_onboarding_form_id=f.id) and lower(coalesce(f.form_code,'')||' '||coalesce(dt.code,'')||' '||coalesce(dt.title,'')) ~ '(гпх|gph|civil|service|оказан.*услуг|подряд|employment_contract|трудов.*договор|договор|contract)')
    )::bigint,
    (
