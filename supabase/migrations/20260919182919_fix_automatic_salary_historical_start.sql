...
begin
  ...
  with visible_employees as materialized (
    select
      ...
      coalesce(employee.monthly_salary, employee.daily_rate, 0)::numeric
        as monthly_salary,
      coalesce(employee.timesheet_excluded, false) as timesheet_excluded,
  ...
  select
    coalesce(
      sum(
        case
          when employee.timesheet_excluded
               and employee.automatic_salary_start_date is not null
            then private.calculate_fixed_monthly_accrual(
              employee.monthly_salary,
              employee.automatic_salary_start_date,
              coalesce(v_first_date, employee.automatic_salary_start_date),
              coalesce(v_last_date, current_date)
            )
          else employee.monthly_salary / 30.0
            * coalesce(attendance.shifts, 0)
        end
      ),
      0
    ),
